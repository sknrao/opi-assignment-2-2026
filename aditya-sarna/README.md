...
- Latest artifact download command:

```bash
gh run download 28868065521 --repo Aditya-Sarna/opi-assignment-2-ovs --name ovs-evidence -D artifacts
```
...
- **Live proof (GitHub Actions — real KVM, `-accel kvm`):** <https://github.com/Aditya-Sarna/opi-assignment-2-ovs/actions/runs/28868065521>
...
| GitHub Actions `ubuntu-latest` | **Real nested KVM:** udev opens `/dev/kvm`, KinD `extraMounts` pass it through, QEMU `-accel kvm`. OVS + ping evidence regenerated every push. **Committed artifacts from [run #28868065521](https://github.com/Aditya-Sarna/opi-assignment-2-ovs/actions/runs/28868065521).** |
